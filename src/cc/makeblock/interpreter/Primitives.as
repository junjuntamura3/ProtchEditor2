...
//		private var counter:int;
		...
		public function addPrimsTo(provider:FunctionProvider):void {
			...
			provider.register("encodeCharactor", encCharFunction); // jun-ichi TAMURA 2017-09-28
			provider.register("decodeCharactor", decCharFunction); // jun-ichi TAMURA 2017-09-28
...
//		}
		...
		/* jun-ichi TAMURA 2017-09-28 */
		private function encCharFunction(thread:Thread, argList:Array):void {
			thread.push(encCharFunctionImpl(argList));
		}
		private function encCharFunctionImpl(argList:Array):String {
			var tValue:String = argList[0];
			var n:Number = Number(argList[1]);
			var oValue:String ;
			var i:int ;
			...
			for (i=0; tValue.length>i ; i++) {
				oValue = oValue + (tValue.charCodeAt(i)+n) + ",";
			}
			return oValue;
		}
		/* jun-ichi TAMURA 2017-09-28 */
		private function decCharFunction(thread:Thread, argList:Array):void {
			thread.push(decCharFunctionImpl(argList));
		}
		private function decCharFunctionImpl(argList:Array):String {
			var tValue:String = argList[0];
			var n:Number = Number(argList[1]);
			var oValue:String ;
			var results:Array
			...
			oValue = "" ;
			results = tValue.split(",");
			for (i=0; results.length>i; i++) {
				if ((results[i] == null) || (results[i] == "")) {
					continue;
				}
				oValue = oValue + String.fromCharCode((results[i]-n));
			}
			return oValue+"";
		}
