package extensions
{
	...
	public class ArduinoUploader
	{
		...
		private var dialogbox:DialogBox2 = new DialogBox2();
			...
			if(mState == ST_DOWNLOADING && downloadProtocol == PROTOCOL_UNO){
				if(bytes[bytes.length-1]==0x10){
					if(prevCmd == STK_LOAD_ADDRESS){
						sendPage(pagelen);
//jun-ichi TAMURA 2017-05-18						dialogbox.setText(Translator.map("Uploading")+" "+Math.floor(hexptr*100/hex.length)+"%");
						dialogbox.setText(Translator.map("Uploading"));
						if(Math.floor(hexptr*100/hex.length)>=100){
							ArduinoManager.sharedManager().isUploading = false;

MBlock.app.scriptsPart.appendMessage("DBUG:parseCmd"); // jun-ichi TAMURA 2017-03-22
...
MBlock.app.scriptsPart.appendMessage("DBUG:uploadHex"); // jun-ichi TAMURA 2017-03-22
